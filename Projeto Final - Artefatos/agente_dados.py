...
def carregar_arquivos_csv(destino_dir):
    ...
    df1 = pd.read_csv(os.path.join(destino_dir, arquivos_csv[0]), dtype=str, encoding="utf-8")
    df2 = pd.read_csv(os.path.join(destino_dir, arquivos_csv[1]), dtype=str, encoding="utf-8")
...
def indexar_em_chroma(df, persist_dir, openai_api_key=None):
    ...
    for _, row in df.iterrows():
        texto = f"""
Data: {row.get('DATA EMISSÃO', 'Não informado')}
